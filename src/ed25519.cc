...
NAN_METHOD(Sign) {
	...
	if ((Buffer::HasInstance(info[1])) && (Buffer::Length(info[1]->ToObject()) == 32)) {
		...
		Handle<Object> privateKeyBuffer = info[1]->ToObject()->Get(Nan::New<String>("privateKey").ToLocalChecked())->ToObject();
...
NAN_METHOD(Verify) {
	...
	info.GetReturnValue().Set(Nan::New(crypto_sign_verify(signatureData, messageData, messageLen, publicKeyData) == 0));
}


void InitModule(Handle<Object> exports) {
	exports->Set(Nan::New("MakeKeypair").ToLocalChecked(), Nan::New<FunctionTemplate>(MakeKeypair)->GetFunction());
	exports->Set(Nan::New("Sign").ToLocalChecked(), Nan::New<FunctionTemplate>(Sign)->GetFunction());
	exports->Set(Nan::New("Verify").ToLocalChecked(), Nan::New<FunctionTemplate>(Verify)->GetFunction());
}
